...
function Dashboard() {
  ...
  const totalVolume = (recent.data ?? []).reduce(
    (sum, r) => sum + r.weight * r.reps,
    0,
  );
  const liveSession =
    recentSessions.find((s) => s.endedAt == null) ?? recentSessions[0];
    ...
    <section className="space-y-6">
      <div>
        ...
        <p className="text-sm opacity-70">
          ...
          {recentSessions.length === 1 ? "" : "s"} · {totalSets} sets logged ·{" "}
          {Math.round(totalVolume).toLocaleString()} lb total volume
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard
          label="The Maxx"
          value={`${Math.round(maxx.upper)}`}
          sub={`${tier.id.toUpperCase()} · ${tier.vibe}`}
          accent
        />
        <StatCard
          label="Total volume"
        ...
        <StatCard
          label="Logged sets"
          value={`${totalSets}`}
          sub={`across ${recentSessions.length} sessions`}
        />
      </div>
          ...
          <ul className="divide-y divide-white/5">
            {recentSets.map((r) => (
              <li
                ...
                className="flex items-center justify-between py-2 text-sm"
              >
                <span className="truncate pr-3 opacity-90">
                  {r.exerciseName}
                </span>
